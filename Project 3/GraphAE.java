...
public class GraphAE<NodeType, EdgeType extends Number>
       ...
       while(!pq.isEmpty()){
            ...
            if(!visited.containsKey(sn.node.data)){
                ...
                for(Edge2 edge : sn.node.edgesLeaving){
                    if((groundClearance>edge.groundClearance)) { //only parse if the car has ground clearance higher than that of the edge.
    ...
    /**
     * Making a modified version of the edge class which extends the orignal edge 
     * class 
     * adds an attribute ground clearance which is the height of the car 
     * 
     */
    ...
    /**
     * modified Node class according to the edited code
     */
